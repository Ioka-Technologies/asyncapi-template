...
/// Ws protocol-specific errors
#[derive(Error, Debug)]
pub enum WsError {
    #[error("Ws connection error: {message}")]
    Connection {
        message: String,
        endpoint: String,
        metadata: ErrorMetadata,
    ...
    #[error("Ws authentication error: {message}")]
    Authentication {
        message: String,
        metadata: ErrorMetadata,
        #[source]
    ...
    #[error("Ws message error: {message}")]
    Message {
        message: String,
...
impl WsError {
    ...
    pub fn metadata(&self) -> &ErrorMetadata {
        match self {
            WsError::Connection { metadata, .. } => metadata,
            WsError::Authentication { metadata, .. } => metadata,
            WsError::Message { metadata, .. } => metadata,
            WsError::Frame { metadata, .. } => metadata,
            WsError::Protocol { metadata, .. } => metadata,
        }
    }
...
impl From<WsError> for AsyncApiError {
...
/// Nats protocol-specific errors
#[derive(Error, Debug)]
pub enum NatsError {
    #[error("Nats connection error: {message}")]
    Connection {
        message: String,
        endpoint: String,
        metadata: ErrorMetadata,
    ...
    #[error("Nats authentication error: {message}")]
    Authentication {
        message: String,
        metadata: ErrorMetadata,
        #[source]
    ...
    #[error("Nats message error: {message}")]
    Message {
        message: String,
        message_id: Option<String>,
        metadata: ErrorMetadata,
...
impl NatsError {
    ...
    pub fn metadata(&self) -> &ErrorMetadata {
        match self {
            NatsError::Connection { metadata, .. } => metadata,
            NatsError::Authentication { metadata, .. } => metadata,
            NatsError::Message { metadata, .. } => metadata,
        }
    }
...
impl From<NatsError> for AsyncApiError {
