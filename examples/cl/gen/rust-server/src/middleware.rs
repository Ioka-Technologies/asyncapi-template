...
impl MiddlewareContext {
    ...
    /// Set a header value in the context
    pub fn set_header(&mut self, key: String, value: String) {
        self.metadata.insert(key, value);
    }

    /// Get all headers from the context
...
/// Authentication middleware that validates tokens and sets claims
#[cfg(feature = "auth")]
...
impl Clone for MiddlewarePipeline {
    fn clone(&self) -> Self {
        // Note: We can't clone the middleware Vec<Box<dyn Middleware>> directly
        // since trait objects don't implement Clone. For now, we'll create a new
        // empty pipeline with the same recovery manager.
        // In a real implementation, you might want to store middleware configurations
        // that can be cloned and recreated.
        Self {
            middlewares: Vec::new(),
...
impl MiddlewarePipeline {
    ...
    /// Add boxed middleware to the pipeline
    pub fn add_boxed_middleware(mut self, middleware: Box<dyn Middleware>) -> Self {
