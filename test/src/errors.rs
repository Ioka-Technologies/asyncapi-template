...
/// Wss protocol-specific errors
#[derive(Error, Debug)]
pub enum WssError {
    #[error("Wss connection error: {message}")]
    Connection {
        message: String,
        endpoint: String,
        metadata: ErrorMetadata,
    ...
    #[error("Wss authentication error: {message}")]
    Authentication {
        message: String,
        metadata: ErrorMetadata,
        #[source]
    ...
    #[error("Wss message error: {message}")]
    Message {
        message: String,
...
impl WssError {
    ...
    pub fn metadata(&self) -> &ErrorMetadata {
        match self {
            WssError::Connection { metadata, .. } => metadata,
            WssError::Authentication { metadata, .. } => metadata,
            WssError::Message { metadata, .. } => metadata,
            WssError::Frame { metadata, .. } => metadata,
            WssError::Protocol { metadata, .. } => metadata,
        }
    }
...
impl From<WssError> for AsyncApiError {
    fn from(error: WssError) -> Self {
